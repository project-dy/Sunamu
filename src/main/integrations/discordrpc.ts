...
function getPresenceConfig() {
	const settings: DiscordPresenceConfig = Object.assign(
		{},
		getConfig<DiscordPresenceConfig>("discordRpc"),
	);
	...
	async function _connect() {
		...
		do {
			...
			try {
				...
				error = true;
				debug(
					"Discord RPC errored out while logging in, waiting 5 seconds before retrying",
				);
...
async function getPresence() {
	if (
		!songdata ||
		!songdata.metadata.id ||
		discordPresenceConfig.blacklist.includes(songdata.appName)
	)
		return;

	const activity: SetActivity = {
		// everything must be two characters long at least
		details: songdata.metadata.title
			? `"${songdata.metadata.title}"`
			: "Unknown track",
		state: songdata.metadata.artist
			? `By ${songdata.metadata.artist}`
			: undefined,
		...
		smallImageText:
			`${songdata.status}` +
			(songdata.metadata.length > 0
				? ` (${secondsToTime(songdata.metadata.length)})`
				: ""),
		instance: false,
		type: 2,
	};
	...
	if (!activity.largeImageKey) activity.largeImageKey = "app_large";
